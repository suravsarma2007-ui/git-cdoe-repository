                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('video.update', $video->id) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="program_id" id="program_id" class="form-select @error('program_id') is-invalid @enderror" required>
                                ...
                                @foreach($programs as $program)
                                    <option value="{{ $program->id }}" @selected(old('program_id', $video->program_id) == $program->id)>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="emp_id" id="emp_id" class="form-select @error('emp_id') is-invalid @enderror" required>
                                ...
                                @foreach($staff as $member)
                                    <option value="{{ $member->id }}" @selected(old('emp_id', $video->emp_id) == $member->id)>
                                        {{ $member->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="module_no" id="module_no" class="form-control @error('module_no') is-invalid @enderror" value="{{ old('module_no', $video->module_no) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="total_videos_required" id="total_videos_required" class="form-control @error('total_videos_required') is-invalid @enderror" value="{{ old('total_videos_required', $video->total_videos_required) }}" min="0" required>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="total_videos_done" id="total_videos_done" class="form-control @error('total_videos_done') is-invalid @enderror" value="{{ old('total_videos_done', $video->total_videos_done) }}" min="0" required>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="total_videos_edited" id="total_videos_edited" class="form-control @error('total_videos_edited') is-invalid @enderror" value="{{ old('total_videos_edited', $video->total_videos_edited) }}" min="0" required>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="uploaded_videos" id="uploaded_videos" class="form-control @error('uploaded_videos') is-invalid @enderror" value="{{ old('uploaded_videos', $video->uploaded_videos) }}" min="0" required>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" name="upload_date" id="upload_date" class="form-control @error('upload_date') is-invalid @enderror" value="{{ old('upload_date', $video->upload_date?->toDateString()) }}" required>
                            @error('upload_date') <span class="invalid-feedback">{{ $message }}</span> @enderror
                        </div>

                        <div class="mb-3">
                            <label for="remark" class="form-label">Remark</label>
                            <textarea name="remark" id="remark" class="form-control @error('remark') is-invalid @enderror" rows="3">{{ old('remark', $video->remark) }}</textarea>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="final_status" id="final_status" class="form-select @error('final_status') is-invalid @enderror">
                                ...
                                <option value="Pending" @selected(old('final_status', $video->final_status) == 'Pending')>Pending</option>
                                <option value="In Progress" @selected(old('final_status', $video->final_status) == 'In Progress')>In Progress</option>
                                <option value="Completed" @selected(old('final_status', $video->final_status) == 'Completed')>Completed</option>
                                <option value="On Hold" @selected(old('final_status', $video->final_status) == 'On Hold')>On Hold</option>
                            </select>
                            @error('final_status') <span class="invalid-feedback">{{ $message }}</span> @enderror
                        </div>

                        <div class="d-flex gap-2">
                            <button type="submit" class="btn btn-success">Update Video Entry</button>
...
<script>
    ...
    async function loadSemesters(programId, selectedSemester = null) {
        ...
        if (programId) {
            try {
                ...
                if (selectedSemester) {
                    loadPapers(programId, selectedSemester);
                }
            } catch (error) {
    ...
    async function loadPapers(programId, semester, selectedPaper = null) {
        ...
        if (programId && semester) {
            try {
                ...
                papers.forEach(paper => {
                    ...
                    if (selectedPaper && paper.id == selectedPaper) {
                        option.selected = true;
                    }
    ...
    semesterSelect.addEventListener('change', function() {
        loadPapers(programSelect.value, this.value);
    });

    // Load on page load
    window.addEventListener('load', function() {
        const initialProgram = '{{ $video->program_id }}';
        const initialSemester = '{{ $video->semester }}';
        const initialPaper = '{{ $video->paper_id }}';
        
        if (initialProgram) {
            loadSemesters(initialProgram, initialSemester);
        }
    });
</script>
@endsection
