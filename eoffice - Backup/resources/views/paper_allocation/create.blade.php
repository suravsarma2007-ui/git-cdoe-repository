...
@section('title', 'Add Allocation - eOffice')

@section('content')
<div class="row">
    <div class="col-md-8 offset-md-2">
        <h2 class="mb-4">Add Paper Allocation</h2>

        <div class="card">
            <div class="card-body">
                <form action="{{ route('paper_allocation.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <select name="paper_id" id="paper_id" class="form-select @error('paper_id') is-invalid @enderror" required>
                            ...
                            @foreach($papers as $p)
                                <option value="{{ $p->id }}" data-program="{{ $p->program->id ?? '' }}" {{ old('paper_id') == $p->id ? 'selected' : '' }}>
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" name="module_no" id="module_no" class="form-control" value="{{ old('module_no') }}">
                        </div>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" name="year" id="year" class="form-control" value="{{ old('year', date('Y')) }}" required min="1900" max="2100">
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="week_no" id="week_no" class="form-select" required>
                                ...
                                @for($w=1;$w<=53;$w++)
                                    <option value="{{ $w }}" {{ old('week_no') == $w ? 'selected' : '' }}>Week {{ $w }}</option>
                                @endfor
                            </select>
                        </div>
