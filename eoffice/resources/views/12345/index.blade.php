    ...
    <div class="table-responsive">
        <table class="table table-bordered table-striped">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $row->id }}</td>
                    <td>{{ \App\Models\Program::getNameById($row->program_id) ?? $row->program_id }}</td>                   
                    <td>{{ \App\Models\Paper::getNameById($row->paper_code) ?? $row->paper_code }}</td>
                     <td>{{ \App\Models\Staff::getNameById($row->emp_id) ?? $row->emp_id }}</td>                 
                    <td>{{ $row->module_no }}</td>
                    <td>{{ $row->status }}</td>
        ...
        <div class="d-flex justify-content-center mt-3">
            @if(method_exists($eslms, 'links'))
                {{ $eslms->appends(request()->except('page'))->links('pagination::bootstrap-5') }}
            @endif
        </div>
    </div>
</div>
